...
public class Giocatore_nico_fill_V extends Giocatore {
	...
	protected char scegliMossaDaFare(int iAvversario, int jAvversario, char[][] scacchiera) {
		...
		// all'inizio scegli a caso se andare su o giu
		if (inizio){
			inizio = false;
			char mossa_iniziale =mosse.charAt(r.nextInt(mosse.length()));
			System.out.println("Mossa iniziale = " + mossa_iniziale);
			return mossa_iniziale;
			}
		
		//poi invece inizia a riempire verticalmente
		if (posY_inizio == 0) {
